...
public class UserService extends BaseService<User, Long> {
    ...
    public User update(User user) {
        ...
        for (int i = 0, l = localUserOrganizationJobs.size(); i < l; i++) {
            ...
            if (dbUserOrganizationJob != null) {//出现在先删除再添加的情况
                ...
                dbUserOrganizationJob.setOrganizationId(localUserOrganizationJob
                    .getOrganizationId());
    ...
    public UserOrganizationJob findUserOrganizationJob(UserOrganizationJob userOrganizationJob) {
        return getUserRepository().findUserOrganization(userOrganizationJob.getUser(),
            userOrganizationJob.getOrganizationId(), userOrganizationJob.getJobId());
    }
    ...
    public User changePassword(User user, String newPassword) {
        user.randomSalt();
        user.setPassword(passwordService.encryptPassword(user.getUsername(), newPassword,
            user.getSalt()));
        update(user);
        return user;
    }
    ...
    public void changeStatus(User opUser, Long[] ids, UserStatus newStatus, String reason) {
        ...
        for (Long id : ids) {
            ...
            UserLogUtils.log(user.getUsername(), "changeStatus", "admin user {} change status!",
                opUser.getUsername());
        }
    }
