...
public class MessageApiImpl implements MessageApi {
    ...
    public Page<Message> findUserMessage(Long userId, MessageState state, Pageable pageable) {
        ...
        //sender的
            ...
            case trash_box:
                ...
                SearchFilter senderFilter = SearchFilterHelper.newCondition("senderId",
                    SearchOperator.eq, userId);
                SearchFilter senderStateFilter = SearchFilterHelper.newCondition("senderState",
                    SearchOperator.eq, state);
                SearchFilter and1 = SearchFilterHelper.and(senderFilter, senderStateFilter);

                //receiver
                SearchFilter receiverFilter = SearchFilterHelper.newCondition("receiverId",
                    SearchOperator.eq, userId);
                SearchFilter receiverStateFilter = SearchFilterHelper.newCondition("receiverState",
                    SearchOperator.eq, state);
    ...
    public List<Message> findAncestorsAndDescendants(Message message) {
        ...
        if (!StringUtils.isEmpty(message.getParentIds())) {
            ...
            filter = SearchFilterHelper.newCondition("parentIds", SearchOperator.prefixLike,
    ...
    public void sendSystemMessageToAllUser(Message message) {
        ...
        do {
            ...
            try {
                ((MessageApiImpl) AopContext.currentProxy()).doSendSystemMessage(page.getContent(),
                    message);
            } catch (Exception e) {
    ...
    private void clearBox(Long userId, MessageState oldState, MessageState newState) {
        if (oldState == MessageState.draft_box || oldState == MessageState.out_box
            || oldState == MessageState.store_box || oldState == MessageState.trash_box) {

            messageService.changeSenderState(userId, oldState, newState);
        }

        if (oldState == MessageState.in_box || oldState == MessageState.store_box
