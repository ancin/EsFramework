...
public class DynamicTaskApiImpl implements DynamicTaskApi {
    ...
    private synchronized void startTask(boolean forceStart, Long... taskDefinitionIds) {
        for (Long taskDefinitionId : taskDefinitionIds) {
            ...
            try {
                ScheduledFuture<?> future = taskScheduler.schedule(createTask(td), new CronTrigger(
                    td.getCron()));
    ...
    private Runnable createTask(TaskDefinition td) {
        ...
        try {
            ...
            if (StringUtils.isNotEmpty(td.getBeanName())) {
                ...
                bean = applicationContext.getAutowireCapableBeanFactory().createBean(
                    Class.forName(td.getBeanClass()));
            }
        ...
        return new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    throw new DynamicTaskException("run dynamic task error, task id is:" + taskId,
                        e);
                }
            }
        };
