...
public class CalendarController extends BaseController<Calendar, Long> {
    ...
    public String showNewForm(@RequestParam(value = "start", required = false) @DateTimeFormat(pattern = dataFormat) Date start,
        ...
        if (start != null) {
            ...
            calendar.setLength((int) Math.ceil(1.0 * (end.getTime() - start.getTime())
                                               / oneDayMillis));
    ...
    @ResponseBody
    public String newCalendar(@ModelAttribute("calendar") Calendar calendar,
