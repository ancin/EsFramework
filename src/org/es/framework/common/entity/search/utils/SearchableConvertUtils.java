package org.es.framework.common.entity.search.utils;
 ...
 * 
 * @author kejun.song
 * @version $Id: SearchableConvertUtils.java, v 0.1 2014年11月19日 上午10:29:42 kejun.song Exp $
 */
public final class SearchableConvertUtils {

    private static final Logger               logger = LoggerFactory
                                                         .getLogger(SearchableConvertUtils.class);
    ...
    public static ConversionService getConversionService() {
        logger.info("#获取ConversionService bean,conversionService=" + conversionService);
        if (conversionService == null) {
            synchronized (SearchableConvertUtils.class) {
                if (conversionService == null) {
                    try {
                        conversionService = SpringUtils.getBean(ConversionService.class);
                        logger.info("#注入ConversionService bean,conversionService="
                                    + conversionService);
                    } catch (Exception e) {
                        logger.error("##获取bean异常:" + e.getMessage());
    ...
    private static void convert(BeanWrapperImpl beanWrapper, Condition condition) {
        ...
        if (isCollection || isArray) {
            ...
            for (int i = 0; i < length; i++) {
                list.set(i,
                    getConvertedValue(beanWrapper, searchProperty, entityProperty, list.get(i)));
            }
            newValue = list;
        } else {
